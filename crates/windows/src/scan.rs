...
mod imp {
    ...
    pub fn scan(_root: &Path, _cross_filesystems: bool, _tx: std::sync::mpsc::Sender<ScanEvent>) -> Result<()> {
        anyhow::bail!("Windows scanner not available on this platform")
    }
}
...
mod imp {
    ...
    fn open_root(root: &Path) -> Result<RootInfo> {
        ...
        let (volume_serial, file_id) =
            get_volume_and_id(&handle).context("failed to get root directory info")?;
        Ok(RootInfo {
            file_id,
            volume_serial,
            name,
    ...
    fn scan_directory(
        ...
        loop {
            ...
            loop {
                ...
                let name_ptr = unsafe {
                    (buffer.as_ptr().add(offset) as *const u8)
                        .add(std::mem::offset_of!(FILE_ID_BOTH_DIR_INFO, FileName))
                        as *const u16
                };
